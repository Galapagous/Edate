...
const ListenOnboarding = () => {
  ...
  const companionProfile = newUser?.currentUser;

  console.log("user -->", newUser);

  if (companionProfile?.profile?.companionPath === "Listener") {
    navigate(`/listen/${companionProfile?.profile?.slug}`);
  } else if (companionProfile?.profile?.companionPath === "Speaker") {
    navigate(`/speaker/${companionProfile?.profile?.slug}`);
  }
    ...
    <div className="w-screen h-screen bg-background_primary p-10">
      ...
      <div className="flex items-center justify-center gap-5 mt-20">
        ...
        >
          ...
          <img
            className={`absolute w-full h-full object-cover transition-opacity duration-500 ${
              changeListener ? "opacity-0" : "opacity-100"
            }`}
            src={Listen2}
          ...
          <img
            className={`absolute w-full h-full object-cover transition-opacity duration-500 ${
              changeListener ? "opacity-100" : "opacity-0"
            }`}
            src={Listen}
            alt="Listener Hover"
          />
          <div className={changeListener ? "text-white" : ""}>
            <h1 className="mt-2 text-lg font-semibold relative z-10">
              Listener
            </h1>
        ...
        <div
          ...
          onClick={() => {
            navigate(`/speak/${companionProfile?.profile?.slug}`);
          }}
          className="cursor-pointer w-[200px] h-[250px] text-center relative overflow-hidden rounded-md"
        >
          {/* Background Image */}
          <img
            className={`absolute w-full h-full object-cover transition-opacity duration-500 ${
              changeSpeaker ? "opacity-0" : "opacity-100"
            }`}
            src={Talk2}
            alt="Speaker Default"
          />
          ...
          <img
            className={`absolute w-full h-full object-cover transition-opacity duration-500 ${
              changeSpeaker ? "opacity-100" : "opacity-0"
            }`}
            src={Talk}
            alt="Speaker Hover"
          />
      ...
      <p className="text-center absolute bottom-0 right-[45%] ml-14">
        Essential {year}
      </p>
    </div>
  );
